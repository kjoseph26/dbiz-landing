...
function WhyDiagram({ active, onSelect }: { active: number; onSelect: (i: number) => void }) {
  ...
  return (
    <svg viewBox='0 0 440 440' xmlns='http://www.w3.org/2000/svg' className='v20-why-diagram'>
      <defs>
        <filter id='v20-why-glow'>
          <feGaussianBlur stdDeviation='4' result='coloredBlur' />
          <feMerge><feMergeNode in='coloredBlur' /><feMergeNode in='SourceGraphic' /></feMerge>
        </filter>
        <filter id='v20-why-glow-sm'>
      ...
      <g clipPath='url(#v20-why-clip)'>
        ...
        {nodes.map((node) => {
          ...
          return (
            <g key={`spoke-${node.idx}`}>
              <line x1={sx} y1={sy} x2={ex} y2={ey} stroke='#F07B2F' strokeWidth={isActive ? 1.5 : 0.8} strokeDasharray='5 4' opacity={isActive ? 0.9 : 0.25} className='v20-why-spoke' />
              <circle r='2' fill='#F07B2F' opacity={isActive ? 0.9 : 0.2} filter='url(#v20-why-glow-sm)'>
        ...
        <circle cx={cx} cy={cy} r={hubR + 6} fill='none' stroke='rgba(240,123,47,0.12)' strokeWidth='0.8' strokeDasharray='3 3' className='v20-why-active-ring' />
        <circle cx={cx} cy={cy} r='18' fill='#E06B1F' className='v20-why-hub-pulse' />
...
export default function WhySection() {
  ...
  return (
    <section className='v20-section' id='about' data-surface='light'>
      <div className='v20-container'>
        <div className='v20-why-head'>
        ...
        <div className='v20-why-interactive'>
          {/* Diagram — center */}
          <div className='v20-why-diagram-wrap'>
            <WhyDiagram active={active} onSelect={setActive} />
          </div>

          {/* Detail panel — right */}
          <div className='v20-why-detail' key={active}>
            <span className='v20-why-detail-idx'>{reason.idx} / {reason.label}</span>
            <h3>{reason.title}</h3>
            <p>{reason.body}</p>
            <a href='#cta' className='v20-why-cta'>Learn more about us <span className='arrow'>→</span></a>
            <div className='v20-why-detail-nav'>
              {reasons.map((_, i) => (
                <button
                  key={i}
                  className={`v20-why-detail-dot ${i === active ? 'active' : ''}`}
                  onClick={() => setActive(i)}
