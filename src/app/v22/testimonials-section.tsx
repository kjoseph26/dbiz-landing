...
export default function TestimonialsSection() {
  ...
  return (
    <section className='v22-quote-section' data-surface='light'>
      <div className='v22-container'>
        ...
        <div className='v22-quote-nav'>
          <button className='v22-quote-arrow' onClick={() => setActive((active - 1 + testimonials.length) % testimonials.length)} aria-label='Previous'>←</button>
          <div className='v22-quote-dots'>
            {testimonials.map((_, i) => (
              <button key={i} className={`v22-quote-dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Testimonial ${i + 1}`} />
            ))}
          </div>
          <button className='v22-quote-arrow' onClick={() => setActive((active + 1) % testimonials.length)} aria-label='Next'>→</button>
        </div>
      </div>
    </section>
  )
