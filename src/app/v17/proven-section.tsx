  ...
  return (
    <section className='v17-section v17-proven' id='work'>
      <div className='v17-container'>
        ...
        {/* Carousel */}
        <div className='v17-proven-carousel'>
          <div className='v17-proven-track' ref={trackRef}>
            {filtered.map((c, i) => (
              <div
                key={c.title}
                className={`v17-proven-card ${i === activeIdx ? 'active' : ''}`}
                onClick={() => setActiveIdx(i)}
              >
                {/* Image area */}
                <div className='v17-proven-img'>
                  <img src={c.image} alt={c.kicker} loading='lazy' />
                  <div className='v17-proven-img-overlay'>
                    <span className='v17-proven-metric'>{c.metric}</span>
                  </div>
                  <div className='v17-proven-img-dots' />
                </div>
                {/* Content */}
                <div className='v17-proven-content'>
                  <span className='v17-proven-kicker'>{c.kicker}</span>
                  <h4>{c.title}</h4>
                  <p>{c.body}</p>
                  {c.techs && (
                    <div className='v17-proven-techs'>
                      {c.techs.map((t) => <span key={t} className='v17-proven-tech-pill'>{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* Navigation */}
          <div className='v17-proven-nav'>
            <button className='v17-proven-arrow' onClick={prev} aria-label='Previous'>←</button>
            <div className='v17-proven-dots'>
              {filtered.map((_, i) => (
                <span key={i} className={`v17-proven-dot ${i === activeIdx ? 'active' : ''}`} onClick={() => setActiveIdx(i)} />
              ))}
            </div>
            <button className='v17-proven-arrow' onClick={next} aria-label='Next'>→</button>
          </div>
        </div>
      </div>
    </section>
