...
export default function TestimonialsSection() {
  ...
  return (
    <section className='v19-quote-section' data-surface='light'>
      <div className='v19-container'>
        <div className='v19-quote-frame' key={active}>
          <div className='v19-quote-meta'>
            <span>N°04 / Testimonial</span>
            <span>{t.company.toUpperCase()}</span>
          </div>
          <div className='v19-quote-mark' />
          <blockquote>{t.quote}</blockquote>
          <cite>{t.name} &nbsp;&mdash;&nbsp; {t.role}, {t.company}</cite>
        </div>
        <div className='v19-quote-nav'>
          <button className='v19-quote-arrow' onClick={() => setActive((active - 1 + testimonials.length) % testimonials.length)} aria-label='Previous'>←</button>
          <div className='v19-quote-dots'>
            {testimonials.map((_, i) => (
              <button key={i} className={`v19-quote-dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Testimonial ${i + 1}`} />
            ))}
          </div>
          <button className='v19-quote-arrow' onClick={() => setActive((active + 1) % testimonials.length)} aria-label='Next'>→</button>
        </div>
      </div>
    </section>
  )
