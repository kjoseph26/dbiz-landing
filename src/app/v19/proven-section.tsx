...
export default function ProvenSection() {
  ...
  return (
    <section className='v19-section v19-proven' id='work'>
      <div className='v19-container'>
        {/* Header row */}
        <div className='v19-proven-header'>
          <div className='v19-proven-header-left'>
        ...
        {/* Carousel */}
        <div className='v19-proven-carousel'>
          <div className='v19-proven-track' ref={trackRef}>
            {filtered.map((c, i) => (
              <div
                key={c.title}
                className={`v19-proven-card ${i === activeIdx ? 'active' : ''}`}
                onClick={() => setActiveIdx(i)}
              >
                {/* Image area */}
                <div className='v19-proven-img'>
                  <img src={c.image} alt={c.kicker} loading='lazy' />
                  <div className='v19-proven-img-overlay'>
                    <span className='v19-proven-metric'>{c.metric}</span>
                  </div>
                  <div className='v19-proven-img-dots' />
                </div>
                {/* Content */}
                <div className='v19-proven-content'>
                  <span className='v19-proven-kicker'>{c.kicker}</span>
                  <h4>{c.title}</h4>
                  <p>{c.body}</p>
                  {c.techs && (
                    <div className='v19-proven-techs'>
                      {c.techs.map((t) => <span key={t} className='v19-proven-tech-pill'>{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
