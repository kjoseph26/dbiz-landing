  ...
  return (
    <section className='v18-section v18-proven' id='work'>
      <div className='v18-container'>
        ...
        {/* Carousel */}
        <div className='v18-proven-carousel'>
          <div className='v18-proven-track' ref={trackRef}>
            {filtered.map((c, i) => (
              <div
                key={c.title}
                className={`v18-proven-card ${i === activeIdx ? 'active' : ''}`}
                onClick={() => setActiveIdx(i)}
              >
                {/* Image area */}
                <div className='v18-proven-img'>
                  <img src={c.image} alt={c.kicker} loading='lazy' />
                  <div className='v18-proven-img-overlay'>
                    <span className='v18-proven-metric'>{c.metric}</span>
                  </div>
                  <div className='v18-proven-img-dots' />
                </div>
                {/* Content */}
                <div className='v18-proven-content'>
                  <span className='v18-proven-kicker'>{c.kicker}</span>
                  <h4>{c.title}</h4>
                  <p>{c.body}</p>
                  {c.techs && (
                    <div className='v18-proven-techs'>
                      {c.techs.map((t) => <span key={t} className='v18-proven-tech-pill'>{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* Navigation */}
          <div className='v18-proven-nav'>
            <button className='v18-proven-arrow' onClick={prev} aria-label='Previous'>←</button>
            <div className='v18-proven-dots'>
              {filtered.map((_, i) => (
                <span key={i} className={`v18-proven-dot ${i === activeIdx ? 'active' : ''}`} onClick={() => setActiveIdx(i)} />
              ))}
            </div>
            <button className='v18-proven-arrow' onClick={next} aria-label='Next'>→</button>
          </div>

          {/* All case studies CTA */}
          <div className='v18-proven-all'>
            <a href='/case-studies' className='v18-proven-all-cta'>
